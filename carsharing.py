...
def get_cars(
    ...
    """ Return all the database cars.
    Query Parameters: size, doors.
    """
    result = db
    if size:
...
def get_car_by_id(
    ...
    """ Return all the database cars, after filtering by id.
    Path Parameters: id.
    """
...
def change_card(id: int, new_data: CarInput) -> CarOutput:
    matches = [car for car in db if car.id  == id]
    if matches:
        car = matches[0]
...
def add_trip(car_id: int, trip: TripInput) -> TripOutput:
    ...
    if matches:
        ...
        new_trip = TripOutput(  id = len(car.trips + 1),
                                start = trip.start, end = trip.end, description = trip.description
        )
